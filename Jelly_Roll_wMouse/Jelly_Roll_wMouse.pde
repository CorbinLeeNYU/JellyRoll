/*Problem description
  Trigger: Play through one time, win game animation happens, game resets to flip book.
  Issues:  1. Countdown stops at "3..." and skips to game
           2. Two blobs appear
           3. An invisible platform exists in the middle of the screen
           4. Backgrounds don't scroll*/
...
float scrollSpeed = 2;     // Speed at which the game scrolls (in pixels/frame)
...
int bgPerLevel = 2;        // Number of times a background image is loaded per level (MUST BE EVEN NUMBER)
...
boolean usingMouse = true; // Set to true when the mouse is to be used instead of the leap as a controller
boolean left, right;       // Used for accelration of the blob when in mouse mode
int picCount = 26;         // Allows user to take multiple screen shots
float blobX;               // Blob's x-pos
float blobY;               // Blob's y-pos
...
void draw() {
  ...
  if (state == 1)        /* In Game */
  {
    ...
    if (!gameOver)
    {
      ...
      } else if (usingMouse) {            // If no right hand is detected, input -1 so the new line
        p.addLine(mouseX, mouseY, false);
      } else {
    ...
    if(frameCount % 80 == 0  && level==1  && !inTransition && !isDead && !gameOver){
      firstLevelObstacle(); 
    }
    else if(frameCount % 60 == 0 && level==2 && !inTransition && !isDead  && !gameOver){
      secondLevelObstacle(); 
    }
    else if(frameCount % 80 == 0 && level == 3 && !inTransition && !isDead  &&!gameOver && !lastBG){
      thirdLevelObstacle(); 
    }
...
void keyPressed()
{
  if (key == ' ') {
    ...
    {
      //saveFrame("jellyRoll" + picCount + ".png");
      //picCount++;
    }
  } else if (key == CODED) {
    if (keyCode == RIGHT) {
