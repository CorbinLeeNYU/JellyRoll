...
int bgPerLevel = 4;        // Number of times a background image is loaded per level (MUST BE EVEN)
...
{ 
  ...
  world.setGravity(0, 100);       // Set the gravity of the world
...
void draw() {
  ...
  if (state == 1)        /* In Game */
  {
    ...
    if (!gameOver)
    {
      ...
      if (rightHand) {                  // If a right hand is detected, use it's
        p.addLine(scrollSpeed, handY);  // y position to make the line
      } else {                          // If no right hand is detected, input -1 so the new line
        p.addLine(scrollSpeed, -1);     // will have the same y val as the most recently added line
      }
    } else
    {
      gameOverAnimation();
    ...
    if(frameCount % 180 == 0  && level==1  && !inTransition && !isDead && !gameOver){
      firstLevelObstacle(); 
    }
    else if(frameCount % 100 == 0 && level==2 && !inTransition && !isDead  && !gameOver){
      secondLevelObstacle(); 
    }
    else if(frameCount % 130 == 0 && level == 3 && !inTransition && !isDead  &&!gameOver){
      thirdLevelObstacle(); 
    }
  ...
  } else if ( state == 2 )        /* Win Screen */
  {
    ...
    if (winGameTimer < int(winGameTime/4))
    {
      image(bg1, bg1x, 0);
...
void displayMoveUpdateBG()
{
  ...
  /* Move the images */
  if (!end)              // Only move the images if we haven't reached the end
  {
    bg1x -= scrollSpeed/2;
    bg2x -= scrollSpeed/2;
...
void gameOverAnimation()
{
  ...
  {
    p.addLine(scrollSpeed, heartBeat(gameOverTimer, height/2, 2*gameOverTime/9));
  } else if (gameOverTimer < gameOverTime)
  {
    p.addLine(scrollSpeed, height/2);
    flatline.play();
  } else
  {
    flatline.pause();
...
float heartBeat(float x, float y, int rate)
{
  if (x%rate < rate - 30)
    return y;
  else if (x%rate < rate - 22)
    return 0;
  else if (x%rate < rate - 10)
  {
    if (isBeep)
    {
      beep.rewind();
